...
def source(name):
    """Path of the STL `name` is loaded from."""
    sub, stem, _ = PLACED[name]
    return os.path.join(HERE, sub, stem + ".stl")


def missing(names):
    """The subset of `names` whose STL is not on disk.

    Both source dirs are gitignored: stl/ is generated by OpenSCAD, and mesh/ holds third-party
    vendor models that are not redistributable (dimensions.json names each one's source).  A
    fresh clone has neither, so callers check first instead of dying in trimesh.load."""
    return [n for n in names if not os.path.exists(source(n))]


STL_HELP = """\
  stl/ is generated, not committed.  Export the printed parts with OpenSCAD (https://openscad.org):
    cd hardware && python3 gen.py && mkdir -p stl
    for p in hub jetson_plate front_plate camera_rocker battery_cradle battery_lid \\
             dock_pin_block pad_carrier; do
      openscad -D "part=\\"$p\\"" -o stl/$p.stl roomba_nx.scad
    done"""

MESH_HELP = ("  mesh/ holds third-party vendor models that are not redistributable, so it is gitignored.\n"
             "  Each one's origin is in the *_source keys of dimensions.json.")
